...
This bound is sharp in the klt category: Lacini constructs, by a modification of the Keel--McKernan construction, a rank-one log del Pezzo surface in characteristic $3$ with exactly seven singular points; see Example~7.4 and the discussion around Corollary~1.2 of \cite{Lacini24}. Thus \cref{thm:main} determines the optimal singular-point bound in this characteristic. The novelty of the present argument is not the existence of the sharp example, but rather the closure of the remaining $N=8$ possibilities.

\medskip
\noindent
\textbf{Proof outline.}
The proof is by contradiction: we assume $N(X)=8$ and derive an impossibility in every case.
The argument has three independent components.
...
\item\emph{Classical algebraic geometry.} The pure Gorenstein basket $8A_1$ is eliminated by the degree-one weak del Pezzo double-cover model: the surface admits a separable double cover of $\mathbb F_2$ whose branch divisor can support at most six nodes.
\end{enumerate}
...
\begin{proof}
The proof is by induction on $m$ and uses only the short exact sequence of a pencil member, so it is characteristic-free (given $\mathrm{char}\ne 2$).

\medskip
\noindent
\textit{Base case $m=1$.}
By Riemann--Roch, $\chi(-K_Y)=2$. By Serre duality, $H^2(-K_Y)\cong H^0(2K_Y)^{\vee}=0$ since $2K_Y$ is anti-effective (it has degree $-2$). Hence $h^0(-K_Y)-h^1(-K_Y)=2$. Since $|-K_Y|$ is a pencil, $h^0(-K_Y)\ge 2$, and since the base locus is a single point (not a divisor), $h^0(-K_Y)\le 2$. Therefore $h^0(-K_Y)=2$ and $h^1(-K_Y)=0$.

\medskip
\noindent
\textit{Inductive step.}
Fix a smooth member $D\in |-K_Y|$ (which exists because the generic member of a genus-one pencil is smooth in characteristic different from $2$). Consider the exact sequence
\[
0\to \mathcal O_Y(-(m-1)K_Y)\to \mathcal O_Y(-mK_Y)\to \mathcal O_D(-mK_Y|_D)\to 0.
\]
By adjunction, $D$ is a smooth curve of arithmetic genus $1$. The restriction $-mK_Y|_D$ has degree $(-mK_Y)\cdot(-K_Y)=m>0=2g(D)-2$, so $H^1(D,\mathcal O_D(-mK_Y|_D))=0$ by the Riemann--Roch theorem on curves. The long exact sequence in cohomology gives
\[
H^1(Y,-{(m-1)}K_Y)\to H^1(Y,-mK_Y)\to H^1(D,-mK_Y|_D)=0.
\]
By the inductive hypothesis, $H^1(Y,-(m-1)K_Y)=0$. Therefore $H^1(Y,-mK_Y)=0$.
...
\]
Blowing up the unique base point $q\in Y$ resolves the Bertini involution and yields a rational elliptic surface $Z=\Bl_qY$. The standard degree-one weak-del-Pezzo theorem identifies $Z$ with a separable double cover of the minimal resolution of $Q$, namely $\mathbb F_2$; see \cite[Prop.~2.2 and Rem.~2.3]{AscherBejleri21} and \cite{HW81}.

The appearance of the negative section $S$ as a branch component is explained as follows. The exceptional curve $E_q$ of the blowup $\sigma\colon Z\to Y$ becomes a bisection of the genus-one fibration on $Z$. Under the double-cover involution, $E_q$ maps to itself and projects to the negative section $S$ on $\mathbb F_2$. Since $E_q$ is pointwise fixed by the involution (it lies over the unique base point of the anticanonical pencil, which is fixed by the Bertini involution), $S$ is a branch component. The remaining branch on $\mathbb F_2$ is a reduced curve $C$ in the class $3M$, where $M=S+2F$ is the pullback of $\mathcal O_Q(1)$.
