...
\newcommand{\repourl}{\texttt{https://github.com/REPLACE-WITH-ORG/nle7-char3-del-pezzo}}
...
In the small-discriminant window, the corrected exact search resolves $45$ baskets; $24$ pass the preliminary exact discriminant test, $21$ fail, and the two historically unresolved baskets are later resolved negatively. After replacing the provisional quadratic-form screen by the corrected odd-lattice bilinear/anticanonical screen, exactly the five baskets listed above remain.
...
\begin{lemma}[Orthogonal real-root subsystems]\label{lem:exotic-root-types}
In the three normal forms of \cref{prop:exotic-normalization}, the orthogonal real-root subsystem is respectively of type
\[
A_7,\qquad A_5,\qquad A_5.
\]
Equivalently, the exact orthogonal root counts are respectively
\[
56,\qquad 30,\qquad 30.
\]
...
\]
which gives a root subsystem of type $A_7$. The exact root enumeration in the repository returns exactly $56$ orthogonal real roots, which is the full root count of $A_7$; hence the orthogonal root subsystem is exactly $A_7$.

For $\frac17(1,2)$ and $\frac1{11}(1,2)$, the corresponding orthogonal real-root sets were enumerated exactly after standard-form normalization. In each case the root graph has exactly $30$ vertices, and the repository records an explicit simple root basis with Cartan matrix of type $A_5$. Since $A_5$ has exactly $30$ roots, these are the complete orthogonal real-root subsystems.

Finally, in a root system of type $A_n$, the maximum number of pairwise orthogonal roots is $\lfloor (n+1)/2\rfloor$. This yields $4$ for $A_7$ and $3$ for $A_5$.
\end{proof}
...
\]
Among these two orbits, only the orbit of size $3360$ is compatible with a transformed noncanonical $5$-clique. Consequently, every mixed-basket lattice configuration in the $L_2$ orbit is Weyl-equivalent to the standard canonical subsystem
\[
S_{\mathrm{std}}=\langle E_3-E_1,\ E_1-E_2,\ E_6-E_4,\ E_5-E_6,\ E_9-E_8\rangle.
\]
\end{theorem}
...
\[
m_1=m_2=m_3,\qquad m_4=m_5=m_6,\qquad m_8=m_9.
\]
...
\]
\[
(E_9-E_8)\cdot D=0\Rightarrow m_8=m_9.
\]
...
In every case the relevant tangent line is forced as a component by \cref{lem:tangent-contact}. Hence each orbit type contains a reducible class and therefore cannot realize the required five irreducible noncanonical exceptional curves.
\end{proof}
...
\]
multisets.
\item Resolve the small-discriminant window exactly. The corrected exact search resolves $45$ baskets in the initial determinant window; $24$ pass and $21$ fail, with two additional unresolved baskets later resolved negatively.
...
For the normalized exotic blocks of \cref{prop:exotic-normalization}, the orthogonal real-root counts are:
\[
56 \text{ roots for } \tfrac17(1,1),
\qquad
30 \text{ roots for } \tfrac17(1,2),
\qquad
30 \text{ roots for } \tfrac1{11}(1,2).
\]
The repository contains the explicit root graphs and simple-root bases used to identify these with $A_7$, $A_5$, and $A_5$ respectively.
...
The GitHub-ready archive accompanying this draft preserves both the paper files and the machine-readable computation summaries. The public repository URL should replace the placeholder
\[
\repourl
\]
in the final submission version.
